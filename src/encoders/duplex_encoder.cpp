...
void DuplexEncoder::encode_antibandwidth()
{
...
void DuplexEncoder::do_encode_antibandwidth()
{
    encode_symmetry_break();

    construct_window_bdds(InstanceData::width);
    glue_window_bdds();
...
{
    number_of_windows = GlobalData::g->n / w;
    last_window_w = GlobalData::g->n % w;
    if (last_window_w != 0)
        number_of_windows++;
    ...
    for (int i = 0; i < GlobalData::g->n; ++i)
    {
        ...
        for (int gw = 0; gw < number_of_windows; ++gw)
        {
            ...
            int p1 = (i * GlobalData::g->n) + (gw * w) + 1;
            int p2 = p1 + w - 1;
            if (last_window)
                p2 = (i + 1) * GlobalData::g->n;
            ...
            {
                InstanceData::cc->add_clause({fwd_amo_id});
            }
        }
        ...
        for (int f = 0; f < (int)fwd_amz_roots[i].size() - 1; ++f)
        {
            ...
            {
                InstanceData::cc->add_clause({fwd_amz_roots[i][f], fwd_amz_roots[i][g]});
            }
        }
        amz_clause.push_back(-1 * fwd_amz_roots[i].back());
        if (!amz_clause.empty())
        {
            InstanceData::cc->add_clause(amz_clause);
        }
    }
};
...
{
    for (int var_group = 0; var_group < GlobalData::g->n; ++var_group)
    {
        ...
        for (int curr_window = 0; curr_window < number_of_windows - 1; ++curr_window)
        {
            ...
            if (fwd_from != fwd_to)
            {
                InstanceData::cc->add_clause({curr_fwd_amo});
            }
            if (bwd_from != bwd_to)
            {
                InstanceData::cc->add_clause({next_bwd_amo});
            }
            ...
            while (fwd_window.size() > 0 && bwd_window.back() <= bwd_from)
            {
                ...
                {
                    InstanceData::cc->add_clause({b1_amo});
                }
                if (bwd_window.size() > 1)
                {
                    InstanceData::cc->add_clause({b2_amo});
                }

                InstanceData::cc->add_clause({b1_amz, b2_amz});
...
void DuplexEncoder::glue_edge_windows(int node1, int node2)
{
    ...
    for (int i = 0; i < (int)node_amz_literals[node1].size(); ++i)
    {
        ...
        for (int c = 0; c < (int)node1_amz_clause.size(); ++c)
        {
            ...
            {
                InstanceData::cc->add_clause({node1_amz_clause[c], node2_amz_clause[d]});
            }
        }
    }
};
...
BDD_id DuplexEncoder::build_amo(std::deque<int> vars)
{
    ...
    {
        new_bdd.id = InstanceData::vh->get_new_var();
        vars.pop_front();
        false_child = build_amo(vars);
        true_child = build_amz(vars);

        InstanceData::cc->add_clause({-1 * from, -1 * new_bdd.id, true_child});
        if (vars.size() > 1)
        {
            InstanceData::cc->add_clause({new_bdd.id * -1, false_child});
        }
    }
...
BDD_id DuplexEncoder::build_amz(std::deque<int> vars)
{
    ...
    else
    {
        ...
        InstanceData::cc->add_clause({-1 * from, -1 * new_bdd.id});
        InstanceData::cc->add_clause({from, -1 * new_bdd.id, false_child});
        InstanceData::cc->add_clause({from, new_bdd.id, -1 * false_child});
    }
...
void DuplexEncoder::make_equal_bdds(BDD_id b1, BDD_id b2)
{
    ...
    if (b1 == -1)
    {
        InstanceData::cc->add_clause({b2});
        return;
    }

    if (b2 == -1)
    {
        InstanceData::cc->add_clause({b1});
        return;
    }

    assert(b1 > 0 && b2 > 0);

    InstanceData::cc->add_clause({-1 * b1, b2});
    InstanceData::cc->add_clause({b1, -1 * b2});
};
